/**
 * @file zoo.hpp
 * @brief Umbrella include for the public zoo-keeper API surface.
 *
 * Consumers can include this header to access the llama.cpp-backed model
 * harness, tool utilities, and the optional hub layer when built with
 * `ZOO_BUILD_HUB=ON`.
 */

#pragma once

// Version
#include "version.hpp"

// Logging
#include "log.hpp"

// Core types and model harness
#include "core/gguf_inspector.hpp"
#include "core/model.hpp"
#include "core/model_info.hpp"
#include "core/system_probe.hpp"
#include "core/types.hpp"
#include "model.hpp"

// Tool system
#include "tools/parser.hpp"
#include "tools/registry.hpp"
#include "tools/types.hpp"
#include "tools/validation.hpp"

// Hub layer (model lifecycle management) — optional
#ifdef ZOO_HUB_ENABLED
#include "hub/huggingface.hpp"
#include "hub/store.hpp"
#include "hub/types.hpp"
#endif
