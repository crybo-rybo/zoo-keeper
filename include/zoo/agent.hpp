...
class Agent {
  public:
     ...
     */
    [[nodiscard]] Expected<void> set_system_prompt(std::string_view prompt,
     ...
     */
    [[nodiscard]] Expected<void> add_system_message(std::string_view message);
    ...
    /// @param timeout Maximum time to wait; returns `RequestTimeout` on expiry.
    [[nodiscard]] Expected<void> clear_history(std::chrono::nanoseconds timeout);

    /// @brief Registers a typed callable as a tool.
    template <typename Func>
    [[nodiscard]] Expected<void>
    register_tool(std::string_view name, std::string_view description,
    ...
    [[nodiscard]] Expected<void>
    register_tool(std::string_view name, std::string_view description, const nlohmann::json& schema,
                  Handler handler, std::optional<std::chrono::nanoseconds> timeout = {}) {
    ...
    [[nodiscard]] Expected<void>
    register_tool(std::string_view name, std::string_view description, nlohmann::json schema,
                  tools::ToolHandler handler, std::optional<std::chrono::nanoseconds> timeout = {});
     ...
     */
    [[nodiscard]] Expected<void>
    register_tools(std::vector<tools::ToolDefinition> definitions,
                   std::optional<std::chrono::nanoseconds> timeout = {});
  ...
  private:
    ...
    RequestHandle<ExtractionResponse> extract_stateful(const nlohmann::json& output_schema,
                                                       ...
                                                       AsyncTokenCallback callback);
    [[nodiscard]] Expected<void>
    register_tool(tools::ToolDefinition definition,
                  std::optional<std::chrono::nanoseconds> timeout = {});
