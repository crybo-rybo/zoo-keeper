...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `on_token` | `optional<TokenCallback>` | empty | Default per-token streaming callback (returns `Continue` or `Stop`). Used only by direct `Model::generate()` calls when no callback is passed directly. `Agent::chat()` uses only its per-request callback parameter |
