...
### HIGH severity
1. ~~**`stop()` deadlocks on a long-running tool handler.** `~AgentRuntime` joins the inference thread, which is blocked on `ToolExecutor::submit(...).get()`. User code can hang the destructor indefinitely. (`src/agent/tool_executor.hpp:30,58`, `src/agent/runtime_inference.cpp:149`)~~ **DONE** — `AgentRuntime` now owns a `CancellationToken`; the inference thread waits on the tool-executor future via `ToolExecutor::wait_for_result()`, which observes both the agent-stop token and the per-request cancellation flag and returns `RequestCancelled` instead of blocking on the handler. (The handler still runs to completion on the executor's worker thread — we cannot kill arbitrary C++ code.)
...
8. **`find_dependency(llama CONFIG)` is unversioned** (`cmake/ZooKeeperConfig.cmake.in:17`) — installed package silently satisfied by mismatched llama versions. **DEFERRED** — initial attempt to pin via `find_dependency(llama 0.0.${LLAMA_BUILD_NUMBER} CONFIG)` was reverted: upstream llama.cpp ships its version file as `llama-version.cmake` rather than the CMake-standard `llama-config-version.cmake`, so CMake reports the installed package as "version: unknown" and any version request fails. Closing this finding requires either an upstream fix or a zoo-keeper install-time rename hack; out of scope for this PR.
