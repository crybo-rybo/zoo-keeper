...
This document records maintainer cleanup findings, recommended fixes, and the
implementation status for the 2.0 line. It is an audit and sequencing document,
not approval for a single large cleanup PR.

The findings below are intended to be worked as small, reviewable changes. The
...
The original audit text remains below for review context. The fixes were landed
as a stacked series of narrow branches, with each branch verified by
`scripts/test.sh` and
`ZOO_INTEGRATION_MODEL=/path/to/model.gguf scripts/crap.sh`
before commit.

| Finding | Branch | Resolution |
| --- | --- | --- |
| H6. llama.cpp archive hash | `codex/maintainer-cleanup-h6-llama-hash` | Default downloads are SHA-256 verified, and custom tags require an explicit hash. |
| H7. Instrumentation leakage | `codex/maintainer-cleanup-h7-private-instrumentation` | Sanitizer, coverage, and CRAP flags stay private to zoo-owned targets. |
| H8. Installed llama identity | `codex/maintainer-cleanup-h8-llama-identity` | Installed packages verify meaningful llama build identity metadata. |
| H9. Hub filename traversal | `codex/maintainer-cleanup-h9-hub-filename` | Explicit HuggingFace filenames are restricted to safe single path segments. |
| H4. JSON grammar correctness | `codex/maintainer-cleanup-h4-grammar-json` | Schema grammar now rejects non-JSON string control characters and non-canonical integer forms. |
| H5. Integral float integers | `codex/maintainer-cleanup-h5-integral-floats` | Tool validation accepts exactly integral finite floats only inside safe integer bounds. |
| H2. Callback lifetime | `codex/maintainer-cleanup-h2-callback-lifetime` | Queued streaming callbacks own stable callback state and report callback failures through request errors. |
| H1. Tool shutdown | `codex/maintainer-cleanup-h1-tool-shutdown` | Tool waits are cancellation-aware; abandoned legacy handlers cannot block runtime shutdown or later tool calls. |
| H3. Multi-tool turns | `codex/maintainer-cleanup-h3-multi-tool-turn` | Every structured tool call in an assistant turn is executed in order with matching tool-result messages. |
| Dead internals | `codex/maintainer-cleanup-m-delete-dead-internals` | Unused runtime hooks and test-only internals were deleted instead of retained as latent API. |
| `ScopeExit` semantics | `codex/maintainer-cleanup-m-scope-exit` | Scope guards are move-construct-only and no longer pay for `std::function` indirection. |
| Hub catalog robustness | `codex/maintainer-cleanup-m-hub-catalog-locking` | Catalog writes use advisory locking and validate catalog versions before mutation. |
| JSON config errors | `codex/maintainer-cleanup-m-json-expected` | Model config loading converts parse and schema failures into `Expected` errors. |
| Auto-config VRAM math | `codex/maintainer-cleanup-m-auto-config-vram` | GPU offload selection reserves context-size-driven KV-cache pressure. |
| Public type shape | `codex/maintainer-cleanup-m-public-type-shape` | Transitional tool-call/message aliases were simplified around the practical public names. |
| Generation result duplication | `codex/maintainer-cleanup-m-generation-result-types` | Agent internals reuse the core generation-result/tool-call response types. |
| Agent command overloads | `codex/maintainer-cleanup-m-agent-command-api` | Void best-effort command helpers and `try_*` aliases were removed in favor of direct `Expected<T>` returns. |
| Core/hub ownership docs | `codex/maintainer-cleanup-m-core-hub-ownership` | Docs now state that GGUF inspection and system probing are core-owned; hub consumes that metadata. |
| Test boundary | `codex/maintainer-cleanup-m-test-boundary` | Unit tests no longer link directly against `llama-common` or include llama headers. |
| Documentation drift | `codex/maintainer-cleanup-docs-drift` | Public docs and this audit were updated to match the completed stack. |
...
## Decisions Captured by the Cleanup

- Tool cancellation remains an internal runtime concern for this stack. A
  public cancellable `TaskContext` can still be introduced later if tool
  handlers need request deadlines, tracing, or cooperative cancellation.
- Legacy blocking handlers run in per-call tasks. If cancellation or shutdown
  abandons a handler, later tool calls are not queued behind that blocked work.
- `max_tool_iterations` continues to count model turns. The runtime now executes
  every tool call within a turn before advancing to the next model turn.
- Core owns GGUF inspection, system probing, model metadata, and JSON config
  loading. Hub consumes those core facilities rather than owning a parallel
  config layer.
- `Agent` command methods now return `Expected<T>` directly. The old
  void-returning best-effort helpers and `try_*` aliases were removed for 2.0.
