...
### HIGH severity
1. ~~**`stop()` deadlocks on a long-running tool handler.** `~AgentRuntime` joins the inference thread, which is blocked on `ToolExecutor::submit(...).get()`. User code can hang the destructor indefinitely. (`src/agent/tool_executor.hpp:30,58`, `src/agent/runtime_inference.cpp:149`)~~ **DONE** — fixed in two layers: (a) `AgentRuntime` owns a `CancellationToken` and the inference thread waits on the tool-executor future via `ToolExecutor::wait_for_result()`, which observes both the agent-stop token and the per-request cancellation flag and returns `RequestCancelled` instead of blocking on the handler; (b) `ToolExecutor` keeps its mutex/queue in a `shared_ptr<Shared>` and `~ToolExecutor` detaches the worker thread instead of joining, so `~AgentRuntime` never blocks on a runaway handler. The handler still runs to completion in the background — we cannot kill arbitrary C++ code — and the detached worker keeps `Shared` alive via the captured shared_ptr.
2. ~~**Streaming-callback use-after-free.** `CallbackDispatcher::dispatch_async` enqueues entries holding raw `AsyncTokenCallback*` into the request slot. On the error-rethrow path the slot can be cleared while the dispatcher thread still holds the pointer. (`src/agent/callback_dispatcher.hpp:111-124`)~~ **DONE** — `handle_request` now always drains the dispatcher before calling `resolve_*`, so no entry can outlive the slot's streaming callback. Streaming-callback exceptions are folded into the resolved error.
3. ~~**Only the first tool call per assistant turn is executed.** `runtime_inference.cpp:108-122` invokes `structured_tool_calls.front()` only; remaining calls are stored in history without execution, breaking multi-call models (Llama-3.1 JSON, Hermes, Qwen3 XML) across turns.~~ **DONE** — tool loop now iterates every structured tool call in a turn, appending one `Tool` message per call; validation failures on one sibling do not abort the others. New `MultipleToolCallsPerTurnAreAllExecuted` test covers the path.
...
6. ~~**Build supply chain: llama.cpp archive fetched without `URL_HASH`** (`cmake/ZooKeeperDependencies.cmake:45-49`). GitHub-generated tarballs are not byte-stable.~~ **DONE** — `ZOO_LLAMA_ARCHIVE_SHA256` is wired as `URL_HASH`; the default tag (`b8992`) ships with a baked-in known-good hash (`942c56b7…6a48`); configure-time `FATAL_ERROR` when downloading without a hash (custom tag without override). Archive base URL switched to `ggml-org`.
...
8. ~~**`find_dependency(llama CONFIG)` is unversioned** (`cmake/ZooKeeperConfig.cmake.in:17`) — installed package silently satisfied by mismatched llama versions.~~ **DONE** — upstream llama.cpp ships its version file as `llama-version.cmake` rather than the CMake-standard `llama-config-version.cmake`, so `find_package` cannot enforce a version constraint natively. Instead, the generated config now loads `${llama_DIR}/llama-version.cmake` explicitly after `find_dependency(llama CONFIG)` and `FATAL_ERROR`s on a `LLAMA_BUILD_NUMBER` mismatch against the value baked in at build time. The check is silent (degrades gracefully) when the upstream version file is absent.
